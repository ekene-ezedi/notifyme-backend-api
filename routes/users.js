  ...
  try {
    ...
    if (!user) {
      ...
      if (!user.isverified) {
        ...
        return res
          .status(400)
          .json({ error: "Account not Verified!", verificationDetails });
      }
...
router.put("/", auth, async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate({ _id: req.user._id }, req.body, {
      new: true,
    }).select("-password -subscriptions -verificationcode -isverified");
...
router.post("/password_reset", async (req, res) => {
  try {
    ...
    // //send email
    const response = await User.sendPasswordResetEmail(
      user,
      passwordResetToken
    );
    if ((response.rejected = [])) {
...
router.put("/:vtoken", async (req, res) => {
  try {
    ...
    let token = jwt.verify(
      req.params.vtoken,
      process.env.PUBLIC_ACCESS_TOKEN_SECRET,
...
router.get("/", auth, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select(
      "-password -subscriptions -verificationcode -isverified"
    );
    res.status(200).json({ user });
  } catch (error) {
    res.status(500).json({ error });
