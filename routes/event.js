...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const events = await Event.find({ channelId: { $in: subscriptions } })
      .populate("channelId", "name imgurl _id")
      .sort("-createdat");
...
router.put("/:id", auth, async (req, res) => {
  try {
    const event = await Event.findOneAndUpdate(
      { admin: req.user._id, _id: req.params.id },
      req.body,
      { new: true }
    );
    res.status(200).json({ success: true, event });
...
router.delete("/guest/:id/:gid", auth, async (req, res) => {
  try {
    const event = await Event.findOneAndUpdate(
      { _id: req.params.id },
      { $pull: { guests: { _id: req.params.gid } } },
      { new: true }
    ).populate("admin", "firstname imgurl");
    res.status(200).json({ success: true, event });
  } catch (error) {
    res.status(500).json({ error });
  }
