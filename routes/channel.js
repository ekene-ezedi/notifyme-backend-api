...
router.get("/subscriptions", auth, async (req, res) => {
  try {
    const channels = await Channel.find({
      subscribers: { $all: [req.user.email] },
    });
    if (!channels) {
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    const channel = await Channel.findByIdAndUpdate(
      { _id: req.params.id, "admin._id": req.user._id },
      data,
      { new: true }
    );
    res.status(200).json({ success: true, channel });
...
router.put("/subscribe/:id", auth, async (req, res) => {
  try {
    ...
    const channel = await Channel.findByIdAndUpdate(
      { _id: req.params.id },
      {
        $addToSet: { subscribers: req.user.email, subscriptions: subscription },
      },
      { new: true }
    );
    ...
    result
      .then(() => res.status(200).json({ success: true, channel }))
...
router.put("/unsubscribe/:id", auth, async (req, res) => {
  try {
    const channel = await Channel.findByIdAndUpdate(
      { _id: req.params.id },
      { $pull: { subscribers: req.user.email } },
      { new: true }
    );
    if (channel) {
      await User.findByIdAndUpdate(
